...
import { PRISME_METRICS_URL, PRISME_PAGEVIEWS_URL } from "../const.ts";

Deno.test("http total request metric", async () => {
  ...
  const expectedValue = Number.parseFloat(
    counterValue(beforeMetrics, "http_requests_total", labels),
  ) + 1;

  expect(counterValue(afterMetrics, "http_requests_total", labels)).toBe(
    expectedValue.toString(),
  );
  expect(
    histogramCountValue(afterMetrics, "http_requests_duration_seconds", labels),
...
function parseMetrics(prometheusMetrics: string): Metrics {
  ...
  for (const line of lines) {
    ...
    if (rawLabels?.length > 0) {
      labels = Object.fromEntries(
        rawLabels.split(",").map((label) => label.split("="))
...
async function fetchAndParseMetrics(): Promise<Metrics> {
  return parseMetrics(await fetchMetrics());
}
