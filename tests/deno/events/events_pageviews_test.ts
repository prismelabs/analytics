...
Deno.test("valid internal pageview traffic with no session associated", async () => {
  const response = await fetch(PRISME_PAGEVIEWS_URL, {
    method: "POST",
    headers: {
      Origin: "http://mywebsite.localhost",
      ...
      "X-Prisme-Document-Referrer":
        "https://mywebsite.localhost/foo?bar=baz#qux",
    },
  });
  await response.body?.cancel();
...
Deno.test("registered domain in Origin header and valid X-Prisme-Referrer is accepted", async () => {
  const response = await fetch(PRISME_PAGEVIEWS_URL, {
    method: "POST",
    headers: {
      Origin: "http://mywebsite.localhost",
...
Deno.test("valid pageview with different Referer and X-Prisme-Referrer headers defined", async () => {
  const response = await fetch(PRISME_PAGEVIEWS_URL, {
    method: "POST",
    headers: {
      Origin: "http://foo.mywebsite.localhost",
...
Deno.test("valid pageview with US IP address", async () => {
  const response = await fetch(PRISME_PAGEVIEWS_URL, {
    ...
    headers: {
      ...
      Referer: "http://foo.mywebsite.localhost/us/foo?bar=baz#qux",
    },
  });
  await response.body?.cancel();
  expect(response.status).toBe(200);
